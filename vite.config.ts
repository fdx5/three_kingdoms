/*
 * 이 파일은 **vite 하나만** import 한다.
 *
 * 예전에는 vitest/config 에서 defineConfig 를 가져왔다. 그러면 게임을 빌드하는 데
 * 테스트 러너까지 필요해진다 — 배포에서 devDependencies 가 빠지면 설정 파일을 읽는
 * 순간 죽는다. 테스트 설정은 vitest.config.ts 로 옮겼다.
 */
import { defineConfig } from 'vite';
import { fileURLToPath, URL } from 'node:url';

export default defineConfig({
  base: './',
  server: {
    watch: {
      /*
       * `img/` 는 감시하지 않는다.
       *
       * 원본 모델(70MB 안팎)을 브라우저로 여기에 직접 받는 작업 흐름이라, 받는 동안
       * `미확인 NNNNN.crdownload` 라는 잠긴 파일이 잠깐 생긴다. vite 의 파일 감시자가
       * 그걸 열려다 EBUSY 로 **프로세스째 죽는다**(실제로 두 번 죽었다).
       * 어차피 vite 가 서빙하지도 번들하지도 않는 폴더다 — 결과물은 asset 파이프라인이
       * `public/assets/models/` 로 낸다.
       */
      ignored: ['**/img/**'],
    },
    /*
     * dev 서버에서도 API 를 같은 오리진으로 쓸 수 있게 넘겨 준다.
     * `npm run server` 를 띄운 뒤 http://localhost:5173/?api=/api 로 열면
     * 배포와 똑같은 경로(토르소 DB)로 돌아간다. 서버를 안 띄우면 이 프록시는
     * 아무 일도 하지 않고, 게임은 localStorage 로 논다.
     */
    proxy: {
      '/api': { target: 'http://localhost:8787', changeOrigin: true },
    },
  },
  resolve: {
    alias: {
      '@': fileURLToPath(new URL('./src', import.meta.url)),
    },
  },
  build: {
    target: 'es2022',
    /*
     * 청크 크기 경고를 1200kB 로 올린다.
     *
     * three.js 를 쓰는 게임이라 메인 번들이 1MB 를 넘는 것은 정상이다(gzip 은 277kB).
     * 기본값 500kB 로 두면 빌드가 성공해도 로그 맨 끝이 항상 노란 경고로 끝나서,
     * 배포가 실패했을 때 그 경고가 원인처럼 보인다 — 실제로 그것 때문에 원인을
     * 두 번 헛짚었다. 경고는 "여기서부터는 진짜 이상한 것"일 때만 떠야 쓸모가 있다.
     */
    chunkSizeWarningLimit: 1200,
    /*
     * 소스맵은 로컬에서만 만든다.
     *
     * three.js 를 통째로 물고 있어 맵이 7MB 나오는데, 배포(render.com 무료 플랜)는
     * 램이 512MB 라 그만큼이 빌드 시간과 메모리를 그대로 잡아먹는다.
     * CI 나 배포에서는 NODE_ENV=production 이 잡히므로 그때는 끈다.
     */
    sourcemap: process.env.NODE_ENV !== 'production',
  },
});
