...
export interface TowerDef {
   ...
   *
   * 체력 자체는 표에 적지 않는다. 바닥값과 건설비가 정하고(BALANCE.towerCombat 의
   * hpBase/hpPerGold) 여기서 종류의 차이만 얹는다: 돌과 쇠로 쌓은 화포 진지는
   * 두껍고, 땅에 깐 철질려 진지는 애초에 부술 것이 별로 없다. 값과 단단함을 따로
   * 적으면 "왜 더 비싼 게 더 잘 부서지나"를 설명할 수 없게 된다.
   *
   * 여기 적힌 값이 그대로 곱해지지는 않는다 — toughnessSpread 가 절반만 반영한다.
   * 종류 간 차이는 순서를 정할 뿐, 어느 망루도 다른 망루의 두 배로 질기지 않다.
   */
  toughness?: number;
