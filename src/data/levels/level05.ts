...
export const LEVEL_05: LevelDef = {
  ...
  startGold: 1000,
  /*
   * 700 -> 1000. 5파에 78기가 한꺼번에 밀려오는 장이라, 횡대가 넓어진 뒤로는
   * 700 으로 시작하면 여섯 슬롯이 1레벨인 채로 그 파를 맞고 그대로 무너졌다(6파 패배).
   * 1000 이면 답(화포)을 세울 시간이 생기고, 그 뒤로도 613/1260 · 누수 255 로 빠듯하다.
   */
  ...
  waves: generateWaves({
    ...
    countStep: 11,
    /*
     * 0.145 -> 0.180. 물량을 2.25배로 늘리면서 한 마리도 두껍게 했다.
     * 실측: 승리하되 성 909/1260, 누수 146 (바꾸기 전 1235/1260, 누수 27).
     * 0.210 에서 8파에 무너지므로 그 아래로만 움직일 것.
     *
     * 이 값을 올린 대가로 **성문 강화가 5장에서도 사실상 필수가 됐다.**
     * 예전에는 "안 올려도 이길 수는 있지만 성이 3분의 1로 깎인다"였는데,
     * 0.155 만 넘어도 안 올리면 진다. 조이면 그 여유가 먼저 사라진다.
     */
    /*
     * 0.1649 -> 0.1600. 타워 업그레이드 곡선을 눕히면서(difficulty.towerLevelFalloff)
     * 이 장이 제일 먼저 무너졌다 — 후반 화력이 줄자 누수가 늘고, 그 골드가 수리로
     * 나가면서 성문을 못 올려 8파에서 도미노가 시작됐다. 전역 가산치까지 더하면
     * 실효 0.165 로, 조이기 전(0.1649)과 사실상 같은 자리다.
     */
    hpGrowth: 0.1600,
