...
export const LEVEL_03: LevelDef = {
  ...
  startGold: 500,
  /*
   * 400 -> 500. 횡대가 넓어진 뒤로는 400 으로 시작하면 일곱 슬롯을 다 세우는 동안
   * 업그레이드가 밀려, 슬롯을 덜 짓는 쪽이 오히려 누수가 적어졌다(실측 25 vs 14).
   * "더 지으면 낫다"가 성립하는 최소선이 500 이다.
   */
  ...
  waves: generateWaves({
    ...
    countStep: 3,
    /*
     * 0.0882 -> 0.0780. 전역 배율(적 체력 x1.15)이 곱해지므로 곡선 자체는 낮춘다 —
     * 그대로 두면 마지막 파에서 원소가 성문에 붙는 순간 뒤집혔다.
     * 지금은 조기 소집 기준 254/500(누수 11), 수리 없이도 66/500 으로 이긴다.
     */
    hpGrowth: 0.0780,
