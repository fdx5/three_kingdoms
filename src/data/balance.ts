...
export const BALANCE = {
   ...
   *
   *                     읽는 법이 하나뿐이다: **화면에 뜨는 숫자가 아니라 체력바에서
   *                     차지하는 비율**이 세기다. 망루 체력을 1/3로 줄였으므로,
   *                     같은 숫자라도 바는 세 배로 움직인다.
   *                     서량 보병(18x3=54)은 한 대에 1.21 — 예전 1.62 보다 작은
   *                     숫자지만, 1레벨 궁노가 600에서 195가 되었으니 바에서는
   *                     0.27% -> 0.62%, 곧 **한 대가 두 배 이상 아프다**.
   *                     여덟이 붙으면 초당 2.8, 1레벨 궁노를 71초에 부순다.
   *                     한 부대가 매달리는 시간(assaultSeconds 20초)에 27% 를 깎으므로
   *                     "한 번 붙으면 끝"도 아니고 "긁히다 만다"도 아니다 —
   *                     세 번 붙으면 무너지고, 그 사이에 고칠 것인지가 판단이 된다.
   *
   *                     왜 더 세게 못 하는가: 이 값과 attackInterval 의 비가 곧
   *                     망루의 수명이고, 수명이 줄면 재건비와 수리비가 업그레이드
   *                     예산을 먹는다. 실측으로 여기서 1.5배만 더 올려도 여섯 장이
   *                     전부 무너졌다(망루를 잃음 -> 화력이 줄고 -> 골드가 줄고 ->
   *                     다시 못 세움). 세기를 더 주고 싶으면 이 값이 아니라
   *                     slots 나 raiderRatio 를 봐야 한다.
   *   bossStrikeDamageMul 장수는 주기가 짧은 만큼 한 대를 작게 나눈다. 그래도
   *                     여포(160x3=480 x 0.0602 = 28.9 / 2.1초)는 혼자서 1레벨
   *                     망루를 14초에, 관우는 8초에 부순다 —
   *                     "장수를 붙게 두면 안 된다".
   ...
   *                     / repairFraction. 즉 **한 번에 최대로 고치면 투자액의 0.13배**,
   *                     덜 깎였으면 그만큼만 낸다(자세한 이유는 Tower.repairCost).
   *                     0.5 에서 0.13 으로 내린 이유는 체력을 줄였기 때문이다.
   *                     수리 한 번이 되돌리는 체력이 1/3이 되었으므로 값을 그대로 두면
   *                     같은 피해를 되돌리는 데 골드가 세 배로 든다 — 실측으로 그때
   *                     봇은 골드를 전부 수리에 태우고 업그레이드도 성문 강화도
   *                     못 했다(4장에서 성문이 영영 1레벨이었다).
  ...
  towerCombat: {
    ...
    strikeDamageMul: 0.0224,
    bossStrikeDamageMul: 0.0602,
    hpBase: 140,
    hpPerGold: 0.55,
    toughnessSpread: 0.5,
    levelHpMul: 1.3,
